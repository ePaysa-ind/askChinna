...
class FirestoreInitializer @Inject constructor() {
    ...
    private suspend fun initializeUsersCollection() {
        try {
            ...
            val adminUser = hashMapOf(
                "name" to "Admin User",
                "phoneNumber" to "+917993754064", // Replace with your actual phone number
            ...
            val adminDoc = db.collection("users").add(adminUser).await()
            Log.d(TAG, "Admin user created with ID: ${adminDoc.id}")
        } catch (e: Exception) {
    ...
    private suspend fun initializeCropsCollection() {
        try {
            ...
            // Add all crops to the crops collection
            for (crop in crops) {
                db.collection("crops").add(crop).await()
            }
    ...
    /**
     * Initialize usageLimits collection for the admin user
     */
    private suspend fun initializeUsageLimitsCollection() {
        try {
            ...
            if (userSnapshot.isEmpty) {
                Log.e(TAG, "Admin user not found, cannot initialize usage limits")
                return
            }

            val adminUserId = userSnapshot.documents[0].id

            // Create usage limits for admin user
            val adminUsageLimits = hashMapOf(
                "remainingUses" to 5,
                "maxUses" to 5,
            ...
            // Add usage limits document
            db.collection("usageLimits").document(adminUserId).set(adminUsageLimits).await()
            Log.d(TAG, "Usage limits created for admin user with ID: $adminUserId")
        } catch (e: Exception) {
